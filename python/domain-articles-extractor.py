...
class DomainArticlesExtractor:
    ...
    def extractArticles(domain):
        procs = []
        websiteArticles = newspaper.build(domain, memoize_articles=False)
        print(len(websiteArticles.articles))

        for article in websiteArticles.articles:
            print(article.url)

        exit(1)
        for article in websiteArticles.articles:
            proc = Process(target=DomainArticlesExtractor.extractArticle, args=(article.url,))
            procs.append(proc)
            proc.start()
            proc.join()

    def extractDomainArticles(self):
        return DomainArticlesExtractor.extractArticles(self.domain)


extract = DomainArticlesExtractor(domain)
print(extract.extractDomainArticles())
